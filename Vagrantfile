...
# db
 ...
 # router
  config.vm.define "router" do |router|
	...
	router.vm.provision "ipforw routing temp", type: "shell", inline: "sudo sysctl -w net.ipv4.ip_forward=1 && sysctl -p "
	#&& iptables -P FORWARD ACCEPT
	router.vm.provision "ipforw routing", type: "shell", privileged: true, inline: <<-SHELL
...
# web
  config.vm.define "web" do |web|
	...
	web.vm.provision "add mount local/files from db to fstab", type: "shell", privileged: true, inline: <<-SHELL
		echo "sshfs#root@db.runalsh.local:/local/files /local/files -o allow_other,ro,IdentityFile=/home/vagrant/.ssh/universal_priv_rsa" >> /etc/fstab
		SHELL
