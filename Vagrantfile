...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", privileged: true, inline: <<-SHELL
	...
	apt-get update
	apt install iproute2 traceroute net-tools nmon wget curl conntrack gpg -y
	sudo mkdir /root/.ssh/
	...
	sudo cp /vagrant/Desktop/epam/universal_priv_rsa /home/vagrant/.ssh/
	...
	cp /vagrant/Desktop/epam/firewall.sh /home/vagrant/ && chmod +x /home/vagrant/firewall.sh	
...
# db
  config.vm.define "db" do |db|
	...
	db.vm.provision "postgre_pgadmin", type: "shell", privileged: false, inline: <<-SHELL
		...
		cp /vagrant/Desktop/epam/db/pgadmin4.db /home/vagrant/ && cp /home/vagrant/pgadmin4.db /var/lib/pgadmin/
		cd /home/vagrant/
		source pgadmin4/bin/activate
		pip install pgadmin4
		cp /vagrant/Desktop/epam/db/pgadmin4.service /home/vagrant/ && sudo cp /home/vagrant/pgadmin4.service /etc/systemd/system/
	...
	db.vm.provision "db_init", type: "shell", privileged: false, inline: <<-SHELL
		...
		cp /vagrant/Desktop/epam/db/{postgremagazines.csv,postgreinit.sql,postgremydb.sql} /home/vagrant/ 
		sudo -u postgres psql -f /home/vagrant/postgreinit.sql
		sudo -u vagrant psql mydb -f /home/vagrant/postgremydb.sql
		sudo cp  /vagrant/Desktop/epam/db/postgresql.conf /home/vagrant/ && sudo cp /home/vagrant/postgresql.conf /etc/postgresql/13/main/
		sudo sh -c 'echo "host mydb vagrant 192.168.10.2/32 md5" >> /etc/postgresql/13/main/pg_hba.conf'
		sudo service postgresql restart
	SHELL
	db.vm.provision "script2_provisioning", type: "shell", privileged: true, inline: <<-SHELL
		cp /vagrant/Desktop/epam/db/{sc2.cfg,sc2.sh,sc2.service} /home/vagrant/
		...
		SHELL
	db.vm.provision "script1_provisioning", type: "shell", inline: "cp /vagrant/Desktop/epam/db/sc1.sh /home/vagrant/ && chmod +x /home/vagrant/sc1.sh"
	db.vm.network "forwarded_port", guest: 5050, host: 5050
 ...
 # router
  config.vm.define "router" do |router|
	...
	router.vm.provision "dns_server", type: "shell", privileged: true, inline: "apt install dnsmasq -y && cp -rf /vagrant/Desktop/epam/router/{dnsmasq.conf,resolv.conf,hosts} /home/vagrant/ && cp -rf /home/vagrant/{dnsmasq.conf,resolv.conf,hosts} /etc && systemctl restart dnsmasq"
...
# elk
  config.vm.define "elk" do |elk|
	...
	elk.vm.provision "log_off_ssl", type: "shell", inline: <<-SHELL 
		...
		cp /vagrant/Desktop/epam/elk/02-beats-input.conf /home/vagrant/
	...
	elk.vm.provision "dns_server_elk", type: "shell", privileged: true, inline: <<-SHELL
		...
		SHELL
	#elk.vm.provision "route", type: "shell", inline: "sh /home/vagrant/firewall.sh -elk"
...
# web
  config.vm.define "web" do |web|
	...
	web.vm.provision "nginx_local", type: "shell", privileged: false, inline: "sudo apt install nginx php-fpm php-pgsql python3 python3-pip ssl-cert libpq-dev ssl-cert -y && pip3 install psycopg2 && sudo mkdir -p /local/scripts && sudo make-ssl-cert generate-default-snakeoil && sudo chown vagrant /local/scripts  && cp /vagrant/Desktop/epam/web/nginx_default /home/vagrant/ && sudo cp /home/vagrant/nginx_default /etc/nginx/sites-available/default && cp /vagrant/Desktop/epam/web/1.php /home/vagrant/ && sudo cp /home/vagrant/1.php /local/scripts && sudo sh -c 'echo extension=pdo_pgsql >> /etc/php/7.4/fpm/php.ini' && sudo sh -c 'echo extension=pgsql >> /etc/php/7.4/fpm/php.ini' && sudo service php7.4-fpm restart && cp /vagrant/Desktop/epam/web/web.runalsh.local.* /home/vagrant/ && sudo sh -c 'cp /home/vagrant/web.runalsh.local.crt /etc/ssl/certs/'  && sudo sh -c 'cp /home/vagrant/web.runalsh.local.key /etc/ssl/private/' && sudo sh -c 'cp /home/vagrant/web.runalsh.local.crt /usr/local/share/ca-certificates' && sudo update-ca-certificates && sudo service nginx restart && sudo sh -c 'cp /usr/share/nginx/html/index.html /local/'"
	web.vm.provision "python_script", type: "shell", privileged: true, inline: <<-SHELL
		pip3 install psycopg2 PrettyTable
		cp /vagrant/Desktop/epam/web/2.py /home/vagrant/ && sudo cp /home/vagrant/2.py /local/scripts
